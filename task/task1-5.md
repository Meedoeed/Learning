...
Остаток времени промучился с тестом на docker-контейнере. Почему-то при создании контейнера и разворачивании в нём pg сервер ложился. При тесте на локальном pg всё работает более чем исправно, пока что меня это удовлетворяет. 

TODO посомтреть по портам pg в контейнере докера, думаю проблема в этом 

Вернулся к фиксу: я понял что локальный pg висит на 5432 и контейнер докера я вешаю туда же, поэтому он и жалуется на то, что пароль не подходит пользователю
Пробую запустить докер на 5433 порту
...
Всё работает.
В daily отчет повесил ряд скринов работы и тестов.